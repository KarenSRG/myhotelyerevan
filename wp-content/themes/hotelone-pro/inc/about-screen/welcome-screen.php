...
class hotelone_dashboard {
	...
	function hotelone_admin_notice() {
		...
		if ( $number_action > 0 ) {
			...
			<div class="updated notice notice-success notice-alt is-dismissible">
				<p><?php printf( __( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our <a href="%2$s">Welcome page</a>', 'hotelone' ),  $theme->Name, admin_url( 'themes.php?page=wd_themepage' )  ); ?></p>
			</div>
			<?php
		}
	}
	...
	function hotelone_admin_import_notice(){
		?>
		<div class="updated notice notice-success notice-alt is-dismissible">
			<p><?php printf( esc_html__( 'Save time by import our demo data, your website will be set up and ready to customize in minutes. %s', 'hotelone' ), '<a class="button button-secondary" href="'.esc_url( add_query_arg( array( 'page' => 'wd_themepage&tab=demo-data-importer' ), admin_url( 'themes.php' ) ) ).'">'.esc_html__( 'Import Demo Data', 'hotelone' ).'</a>'  ); ?></p>
		</div>
		<?php
	}
	...
	function hotelone_admin_dismiss_actions(){
		...
		if ( isset( $_GET['hotelone_action_notice'] ) ) {
			...
			$action_key = sanitize_text_field( $_GET['hotelone_action_notice'] );
	...
	public function hotelone_theme_about_page(){
		...
		if ( isset( $_GET['hotelone_action_dismiss'] ) ) {
			...
			$actions_dismiss[ sanitize_text_field( $_GET['hotelone_action_dismiss'] ) ] = 'dismiss';
			update_option( 'hotelone_actions_dismiss', $actions_dismiss );
		}
		...
		<div class="wrap about-wrap themepage_wrapper">
			...
			<h1>
				<?php printf( esc_html__('Welcome to Hotelone - Version %1s', 'hotelone'), $theme->Version ); ?>
			</h1>
			
			<div class="about-text">
				<?php esc_html_e( 'Hotelone is a creative and flexible WordPress ONE PAGE theme well suited for business, portfolio, digital agency, product showcase, freelancers websites.', 'hotelone' ); ?>
			</div>
			
			<a target="_blank" href="<?php echo esc_url('https://www.britetechs.com/'); ?>" class="britetechs-badge wp-badge"><span>Britetechs</span></a>
					...
					<div class="themepage_info_column clearfix">
						<div class="themepage_info_left">
							...
							<div class="themepage_link">
								...
								<p class="about"><?php printf(esc_html__('%s supports the Theme Customizer option panel settings. Click on below customize button to customize your theme.', 'hotelone'), $theme->Name); ?></p>
								<p>
									<a href="<?php echo admin_url('customize.php'); ?>" class="button button-primary"><?php esc_html_e('Customize Your Theme', 'hotelone'); ?></a>
								</p>
							</div>
							<div class="themepage_link">
								<h3><?php esc_html_e( 'Documentation', 'hotelone' ); ?></h3>
								<p class="about"><?php printf(esc_html__('Need to setup your %s WordPress theme? Please click on bottom button  to find the theme documentation.', 'hotelone'), $theme->Name); ?></p>
								<p>
									<a href="<?php echo esc_url( 'http://docs.famethemes.com/category/112-hotelone' ); ?>" target="_blank" class="button button-secondary"><?php esc_html_e('Hotelone Theme Documentation', 'hotelone'); ?></a>
								</p>
								<?php do_action( 'hotelone_dashboard_theme_links' ); ?>
							</div>
							<div class="themepage_link">
								<h3><?php esc_html_e( 'Need Support?', 'hotelone' ); ?></h3>
								<p class="about"><?php printf(esc_html__('If you have more queries with %s WordPress theme. Please click below link to create a ticket', 'hotelone'), $theme->Name); ?></p>
								<p>
									<a href="<?php echo esc_url('https://www.britetechs.com/' ); ?>" target="_blank" class="button button-secondary"><?php echo sprintf( esc_html__('Create a ticket', 'hotelone'), $theme->Name); ?></a>
								</p>
							</div>
						</div>

						<div class="themepage_info_right">
							<img src="<?php echo get_template_directory_uri(); ?>/screenshot.png" alt="Hotelone Theme Screen" />
						</div>
					</div>
				</div><!-- tab 1 -->
			<?php } ?>
			...
			<?php if ( $tab == 'recommended_actions' ) { ?>
				<div class="action-required-tab info-tab-content">
					...
					<?php if ( $actions_r['number_active']  > 0 ) { ?>
						...
						<?php if ( $actions['recommend_plugins'] == 'active' ) {  ?>
							<div id="plugin-filter" class="recommend-plugins action-required">
								<a  title="" class="dismiss" href="<?php echo add_query_arg( array( 'hotelone_action_notice' => 'recommend_plugins' ), $current_action_link ); ?>">
						...
						<?php if ( $actions['page_on_front'] == 'active' ) {  ?>
							<div class="theme_link  action-required">
								<a title="<?php  esc_attr_e( 'Dismiss', 'hotelone' ); ?>" class="dismiss" href="<?php echo add_query_arg( array( 'hotelone_action_notice' => 'page_on_front' ), $current_action_link ); ?>">
								...
								<div class="about">
									<p><?php _e( 'In order to have the one page look for your website, please go to Customize -&gt; Static Front Page and switch "Front page displays" to "A static page".', 'hotelone' ); ?></p>
								</div>
								<p>
									<a  href="<?php echo admin_url('options-reading.php'); ?>" class="button"><?php esc_html_e('Setup front page displays', 'hotelone'); ?></a>
								</p>
							</div>
						<?php } ?>
						...
						<?php if ( $actions['page_template'] == 'active' ) {  ?>
							<div class="theme_link  action-required">
								<a  title="<?php  esc_attr_e( 'Dismiss', 'hotelone' ); ?>" class="dismiss" href="<?php echo add_query_arg( array( 'hotelone_action_notice' => 'page_template' ), $current_action_link ); ?>">
								...
								<p>
									...
									if ( $front_page > 0 && get_post_meta( $front_page, '_wp_page_template', true ) != 'template-frontpage.php' ) {
										?>
										<a href="<?php echo get_edit_post_link( $front_page ); ?>" class="button"><?php esc_html_e('Change homepage page template', 'hotelone'); ?></a>
										<?php
									}
									?>
								</p>
					...
					<?php  } else { ?>
						...
						<h3><?php  printf( __( 'Keep %s updated', 'hotelone' ) , $theme->Name ); ?></h3>
						
						<p><?php _e( 'Hey! There are no required actions found.', 'hotelone' ); ?></p>
			...
			<?php if ( $tab == 'demo-data-importer' ) { ?>
				<div class="demo-import-tab-content info-tab-content">
					...
					<?php if ( class_exists('OCDI_Plugin') ) {?>
						...
						<?php printf(sprintf(__( '<p>Congratulations! you installed importer plugin successfully. Click Here to start <a href="'.esc_url( admin_url('themes.php?page=pt-one-click-demo-import') ).'">Import Data</a></p>', 'hotelone' ))); ?>
						</div>
