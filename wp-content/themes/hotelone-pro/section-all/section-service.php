...
$service_title    = get_theme_mod( 'hotelone_services_title', wp_kses_post('Our <span>Services</span>','hotelone') );
$service_subtitle    = get_theme_mod( 'hotelone_services_subtitle', wp_kses_post('What we provide to the customers','hotelone') );
		...
		<div class="row">
			...
			foreach ($page_ids as $settings) {
				...
				if ( $settings['icon'] ) {
					...
					$media = '<p><i class="fa '.esc_attr( $settings['icon'] ).'"></i></p>';
				}
				if ( $layout == 12 ) {
